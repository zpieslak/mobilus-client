...
class TestMQTTClient(unittest.TestCase):
    ...
    def test_send_request_with_login_request(
            self, mock_publish: Mock, mock_is_connected: Mock, _mock_time: Mock) -> None:
        self.client.send_request("login", login="user", password=self.config.user_key)
    ...
    def test_on_disconnect(self, mock_info: Mock) -> None:
        self.client.on_disconnect(Mock(), None, 0)

        mock_info.assert_called_once_with(ANY, "Disconnected with result code - 0")
